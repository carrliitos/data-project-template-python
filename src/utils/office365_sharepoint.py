...
def upload_files(local_folder, sharepoint_folder_name, keyword=None):
    """
    Upload files from a local folder to a SharePoint folder based on specified criteria.

    Args:
    - local_folder (str): The path to the local folder containing the files to upload.
    - sharepoint_folder_name (str): The name of the SharePoint folder to upload to.
    - keyword (str): The pattern to match in file names. Use None to upload all files.
    """
    ...
    for file in files_list:
        if keyword is None or keyword == 'None' or re.search(keyword, file[0]):
            ...
            SharePoint().upload_file(file_name=file[0], folder_name=sharepoint_folder_name, content=file_content)

def download(sharepoint_site_folder_name, folder_dest, file_name, file_name_pattern):
    """
    ...
    Args:
    - sharepoint_site_folder_name (str): The SharePoint folder name. May include subfolders (e.g., Tableau/Workbooks).
    - folder_dest (str): The destination folder where files will be saved.
    - file_name (str): The specific file name to download. Use None to download all files.
    - file_name_pattern (str): The pattern to match in file names. Use None to download all files.
    """
    if file_name:
        get_file(file_name, sharepoint_site_folder_name, folder_dest)
    elif file_name_pattern:
        get_files_by_pattern(file_name_pattern, sharepoint_site_folder_name, folder_dest)
    else:
...
def upload(root_dir, sharepoint_folder_name, filename_pattern):
    ...
    You can specify a filename pattern to upload only files that match the pattern.
    ...
    - sharepoint_folder_name (str): The SharePoint folder name. May include subfolders to upload to.
